...
|---|---|---|---|---|
| ASSAY-FIND-1 | **Banded honesty survives re-derivation and strengthens**: REMIT holds the line at rendering; ASSAY found the rendering-level rule insufficient and pushed the ban into *representation* — no stored midpoint, no distributional claim, anywhere (DEC-15), on the argument that a stored midpoint *will* eventually surface. | DEC-9, DEC-15; knowledge model §3 ("no midpoint slot exists and none may be added") | NF9/NF10 (rendering-level, per DEC-9's citation) | Convergent-and-strengthened · **REMIT register candidate**: adopt the representation-level ban — audit REMIT shapes for stored midpoints/derived scalars |
| ASSAY-FIND-2 | **Numeric priority weights did not survive re-derivation** where commander judgement is the subject: ASSAY refused numeric commitment weights entirely — ordinal tiers plus *stated* tie-breaks (DEC-19) — on the argument that numeric sacrifice weights are false precision exactly where scrutiny is highest. | DEC-19; knowledge model §5 ("no numeric weight slot exists"); seam G4 tie-break rule | To cite: how REMIT weights competing requirements at its argument surface | Divergent-or-qualifying · **pending** (owner: design team) — if REMIT carries numeric weights on judgement-owned orderings, this finding challenges them |
...
| ASSAY-FIND-4 | **Deterministic demonstrators must banish wall clock from stored objects**: scenario-clock-only time (DEC-17) was forced by stamp determinism and offline reproducibility, not by doctrine — suggesting it generalises to any system claiming replayable computation. | DEC-17; knowledge model §1.4; seam G1 | To cite: REMIT's time handling in stored/computed objects | Pending comparison · **pending** (owner: design team) |
...
1. The two `pending` entries (FIND-2, FIND-4) need their REMIT-side citations — a design-team task, bounded to looking up two register entries.

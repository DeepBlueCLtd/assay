...
Status: **built 2026-07-13** (v0.2) · this proposal is now realised as the **SPEC-14 flow-view sub-slice** (`specs/014-flow-view/`; `src/flow.ts` + `src/flowPage.ts` + `scripts/build-flow.ts`, `npm run flow`; `tests/flow.test.ts` turns §9's AS-1…AS-11 into real tests). The interactive view ships as one self-contained embed (`docs/assets/flow/index.html`) in two homes — the in-app S4 "systems-map" mode and the public Pages explainer (`site/flow.html`), linked from an `assay-home.html` card. The four behavioural candidates (concept §6.15) were built under delegated authority; ratify-after debt is owed in the next register batch. Original status: draft for review · candidate addition to the canonical set · v0.2 adds the implementation-facing sections (§7 interaction contract, §8 view-state & tour script, §9 acceptance intent, §10 implementation notes & build-stage dependencies) so the proposal could be picked up cold in a later session
...
- `assay-flow-schematic-wireframes.html` — the **engineering-drawing / systems-schematic** form: the same heartbeat drawn as one apparatus (knowledge piped in → `compile` → `score` → `relax` → `select`), with the six gates as inline decision valves, bands on gauges, refusals as honest-outcome terminals, and a drawing title block. Organised by *the flow itself* — the "motor-engine / power-station" reading of the same node-link topology the research note (07 §3) justifies. Carries a working **Tour mode** (this spec §4.2 / §8): a wizard (Back / Next / step-dots / Play) steps the six-beat walkthrough, animating data through the conduits, pulsing the gate that fires, and landing each honest consequence — the contest refusal, the attribution-visible stamp-flip-plus-delta (§2.2), the least-worst frontier — with a **Full schematic** toggle for the static overview. The tour is a scripted, oracle-consistent replay (research note 07 §5), and the staleness beat is labelled scripted; the sandbox (§4.3) and computed recompute are not built here.
...
- **Process (research-first, DEC-11).** The interactive view is a surface with seam calls, so implementation opens on a research note. **That note has landed: `docs/research/07-flow-view.md`** (interaction/animation approach; the fan-out-vs-recompute distinction; the auto-recompute honesty question) — DEC-11 satisfied, the gate is clear. This document is the *proposal*; it seeds the spec-kit SPEC (SPEC-14 site/gallery neighbourhood, or its own SPEC-## when scheduled) but is not itself that SPEC.
...
- **Research-first (DEC-11) is not waived by that delegation — and is now satisfied.** The note **`docs/research/07-flow-view.md` has landed** (the interaction/animation approach, the fan-out-vs-recompute distinction, and the auto-recompute honesty question), exactly as notes `01`/`02` preceded the schema changes they carried. The gate is clear; implementation may open on it.

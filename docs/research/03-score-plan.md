...
**Where this sits in prior art** *(added 2026-07-15, citation-hardening pass — review A2)*. The mapping has kin it should name. Reading a claim off an interval's two endpoints separately is possibility theory's paired measures — `m_hi ≥ 0` says satisfaction is *possible*, `m_lo ≥ 0` says it is *necessary* (Dubois & Prade), and the four stops are exactly the consistent (necessity, possibility) sign patterns. "Robust iff the worst realisation clears the requirement" is info-gap's robust-satisficing posture (Ben-Haim). And the conservative interval comparison underneath §5's non-domination is the classical interval-order idiom (Fishburn). The O-3 uniqueness argument stands on its own; these situate the shape rather than derive it.

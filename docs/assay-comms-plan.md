...
Status: draft for review · v0.5 · 2026-07-13 (v0.2 adds §6: blog articles with working interactive embeds; v0.3 adds §12: the SME listening loop, and the schema namespace on the Documents page; v0.4 wires the per-spec home-page-currency step into §9 — flagged candidate, concept §6.12, pending ratification — and the `docs/assay-home.html` Home page it maintains; v0.5 lands the blog + standalone-embed scaffold and the `docs/status.yml` single source of truth, records the Jekyll→static-assembler build-path divergence in §8, and flags concept §6.14 — the blog cadence as register-blessed authoring work, not a spec-kit feature)
...
6. **Every shippable UI artefact is reachable by clicking.** Any surface, wireframe, or explainer that ships in a build must be **both copied into the site and linked from a navigable page** (the Home page cards). A reviewer — SME or maintainer — must reach it from the per-PR preview by clicking, never by hand-typing a URL. A page present at a URL but linked from nowhere is findable only by its author; that is a reachability form of the optimism drift principle 2 forbids, and `scripts/build-site.ts` records the rule at the copy site so it is not silently skipped when the next artefact lands.
...
> **Process note (flagged candidate, concept §6.14 — pending ratification).** A blog article is *not* a spec-kit feature. It is authoring work that *projects* an already-shipped slice — the slice went through the full `specify → plan → tasks` machinery; the article does not get its own pass. This is ASSAY-DEC-12's rule (documents are authoring, not spec-kit) extended from founding documents to communications. Articles are owed at a stage/spec gate (§9), tracked in `docs/blog/backlog.md`, and authored under comms honesty — never carved into a numbered SPEC.
...
> **Adopted in practice (2026-07-13, flagged candidate concept §6.14).** The §8 recommendation below floated **Jekyll**; the build settled on a **hand-rolled static assembler** — `scripts/build-site.ts` (`npm run build:site`) copies the hand-authored Home page, the fixture-backed gallery, the wireframes, and `docs/blog/` into a `site/` root that GitHub Actions publishes to `gh-pages` (with per-PR previews). Reasons: it reuses the exact static approach already used for the Home page and the gallery (one build path, no Jekyll toolchain), every page is self-contained static HTML that works offline, and `docs/status.yml` is read by the assembler directly (which also lets it *enforce* the §5 honesty gates at build time). Where the bullets below say "Jekyll", read "the static assembler"; the *intent* of each bullet holds unchanged.

- **Hosting:** GitHub Pages, published to `gh-pages` by `scripts/build-site.ts` (site and canonical docs stay co-located in `/docs`; the generated `site/` is git-ignored). Public, per the SME-facing decision.
- **Generator:** the hand-rolled static assembler (`build-site.ts`) — zero framework. It reads `docs/status.yml` directly and emits `site/status.json`, so the tracker is data-driven with no client JavaScript; it also *fails the build* if the Home page's stage label drifts from `status.yml` or a stage claims `building`/`done` with its research note unpublished (§5). ~~Jekyll + data plugin~~ superseded, see the note above.
- **Blog:** self-contained static posts under `docs/blog/posts/`, listed newest-first from `docs/blog/index.html` (and the Roadmap updates feed); an updates-feed entry either is its own one-liner or links to its full article. Authoring + embed convention: `docs/blog/README.md`. ~~Jekyll `_posts/`~~ superseded.
- **Interactive embeds (§6):** each embed is a self-contained client-side bundle under `docs/blog/embeds/<component>/`, mounted into its post via a single `<iframe>` (the honest form of "a `<div>` plus one `<script>`" for an embed that is its own page). Component embeds are a *build product* of the standalone-embed target `scripts/build-embeds.ts` (`npm run embeds`) — it imports the shipped SPEC-14 component and pre-renders its reachable states over inlined Meridian fixtures, never re-implementing it. Algorithm widgets are small modules kept next to the article. Only articles carry embeds.
- **Wireframes:** `assay-ui-wireframes.html` (the four role surfaces) and `assay-flow-infographic-wireframes.html` (the proposed system-flow explainer) are copied as static assets (`wireframes.html`, `flow-wireframes.html`) and linked from the Home page cards — copied *and* linked, per §1.6, so every preview surfaces them without a typed URL.
- **Deploy:** a GitHub Actions build runs `npm run build:site` and publishes `site/` to `gh-pages`, preserving per-PR preview subtrees (`.github/workflows/deploy-pages.yml`, `pr-preview.yml`).
...
- **On every stage gate** (exit criteria met in the build plan): update `docs/status.yml` (the single source of truth, §5), publish the stage's research note to the Roadmap index, add an updates-feed entry. Where the stage introduced a new component or a graphically-expressible algorithm, that entry is a full blog article carrying its working embed (§6) — choosing and shipping the embed is part of authoring the article, not a separate task, and the article is authoring work, not a spec-kit feature (§6 process note; flagged candidate concept §6.14). This ties site currency to the build's own gates — no separate reporting ritual. Articles owed by gates that landed before the blog vehicle existed are tracked as a queued debt in `docs/blog/backlog.md`, drawn down to zero.

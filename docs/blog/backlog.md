...
Status: working document · 2026-07-13 · dev-facing (not published — `build-site.ts` skips `.md`)
...
|---|---|---|---|---|
| 0 | SPEC-01 store | Content addressing — edit a small object, watch its hash (identity) change | Yes — `src/store` hash is shipped | **owed** |
| 0 | SPEC-02 trace store | Trace walk — click a node, see forward/backward closure light up | Yes — trace graph shipped | **owed** |
...
| 1 | SPEC-05 knowledge | The provenance chip & the K10 refusal — markings that behave; a refusal that is legible as discipline | Yes — `provenanceChip`, `refusalBanner` shipped | **owed** |
| 2 | SPEC-06 compile | Stamp determinism — recompile twice for a byte-identical stamp; change one item and watch it shift | Yes — compile + stamp shipped | **owed** |
| 3 | SPEC-07 scorer | Verdict playground — move a commitment threshold across a banded score; the four-stop verdict flips only at band edges | Yes — scorer + `s2Matrix` shipped | **owed** |
...
| 4 | SPEC-09 relaxation | Least-worst explorer — choose which commitment to sacrifice; see what each relaxation buys and costs | Yes — relax + `s3Cards` shipped | **owed** |

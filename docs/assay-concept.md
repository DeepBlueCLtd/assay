...
21. **Spatial & temporal COA visualisation — a first-class, honest, interactive surface** *(raised 2026-07-14; spec `specs/019-coa-visualisation/`; awaiting research note `docs/research/10-spatial-temporal.md` under DEC-11, then SME review under comms §12)*. Every COA surface today is tabular; yet a verdict *is* a route walking through a banded channel surface over time (SPEC-18 states this in prose but nothing renders the geometry). SPEC-19 extends the flow-view map schematic (**ASSAY-DEC-31(c)**, already ratified) from a static panel to a temporal, interactive COA surface rendered from the real fixture geometry (region boxes, route waypoints, sparse channels). Sub-questions for a future batch: **(a)** the **banded-surface rendering convention** — how a channel band renders *as a band* (extent/two-stop/textured, hover always yielding exact `[lo,hi]`), never a single heat stop implying a point value (the crux; touches G2/DEC-15); **(b)** the **temporal convention** — validity/task windows as discrete extents with lapsed channels marked, never a band interpolated into an implied-certainty curve (touches thesis F); **(c)** a **drag-to-recompute geometry-edit affordance** — dragging a route waypoint / shifting a task window authors a new `Plan` version and re-scores through the live pipeline (input, not just understanding), honest because it edits geometry not the answer and reuses the SPEC-16 recompute + glow (touches DEC-20/21, G1, G6); **(d)** whether the surface promotes to a standing home (S2/S4 mode or a fifth tab) — **deferred to after the US1 SME review**, with DEC-31(d)'s "never silently optimise" caution binding any auto-behaviour. Through-line: a map is the most tempting place to launder banded intelligence into false precision, so the reveal this surface adds must be the *honest gap itself* (a band reaches a verdict only where a metric reads the channel **and** the route enters the region — K6→C4 propagates, K7→C4 misses, K3→C3 is geometric-only), never a prettier claim. Register-first (DEC-2): no schema or code decision until ratified; research-first (DEC-11): the note is the gate. Sibling to the flow view (DEC-31) and to issue #24 (dependency-graph view — a different focus, both post-SPEC-16 presentation slices).

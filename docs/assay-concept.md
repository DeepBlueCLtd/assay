...
27. **The decision-point derivation rule — the DSM's semantics (SPEC-24)** *(flagged 2026-07-16; review §4.2/M1/B1 + addendum slice S-D; decided technically by research note `12-decision-support.md` under the DEC-11 gate)*. A ranking/semantics decision of DEC-18's class: a commitment on a plan is a **decision point** iff its verdict is **scenario-divergent** (some adversary-COA pair in the SPEC-10 tensor flips it — the row's own operative pairs, verdict divergence only, K14 never enters) or **margin-class** (`tight`/`marginal` under the selected world); uniform `robust` and uniform `violated` are explicitly non-DPs (the latter is a plan defect — relax's business, G4). The **commit step** is derived from plan geometry via the metric's own reads (first divergent-read leg's `enter_step`; horizon step, worded *world-decided*, for `state` metrics; stated absence for margin-class-only rows). **LTIOV = commit_step − lead (lead 0 in v1, stated)** on the scenario clock only (DEC-17), with a three-state answerable-in-time predicate — in time (slack a step count), **red with visible arithmetic** (never dropped, G4's spirit), unstated (no `earliest_result` is never assumed answerable). Discriminators reuse SPEC-12/23 classification (nested excluded); tripwires are consumed knowledge lapsing before the commit step, at the trace graph's honest world-level granularity. No urgency/priority/risk scalar anywhere (DEC-19); row order is stated presentation order. Decided in detail by the note (DEC-11 gate); built under SPEC-24; awaiting ratification in the next register batch.

28. **The DSM surface and seam addition — `POST /analyse/decision-support` (SPEC-24)** *(flagged 2026-07-16; review §4.2/M1 + addendum slice S-D)*. DEC-24-style contract growth: a new analysis endpoint (thin orchestration — tensor + discrimination classification + validity windows + plan geometry, no new engine, DEC-10) returning derived `DecisionPointRow`s, stamped and `cited_in`-edged to its evidence (G3); and a new **derived surface** — the note recommends an S2-adjacent **"Decisions in time" panel on the commander tab**, not a sixth tab (DEC-32's four role tabs stand unextended; the DSM is a commander/J-3 projection of the same tensor the scenario strip renders). No tasking affordance — collection options render with cost bands and traces only; tasking remains a human act with its own commitment consequences (the walkthrough's KINGFISHER/C6 discipline). No schema change (movement types only). Candidate only — recorded here per DEC-2; SPEC-24 may not be cited as authority.

29. **The focused transitive dependency-graph surface (issue #24)** *(flagged 2026-07-16; review §4.2 C4 + addendum §10 slice S-G, the full-screen big brother of S-G's recursive trace tooltips; decided technically by research note `13-dependency-graph.md` under the DEC-11 gate)*. DEC-24/DEC-28-class contract growth: a **derived, on-demand surface** — pick one item (K/C/R/P/world/verdict) and read its **entire** upstream (informs) and downstream (influences) chains at once, rendered as a depth-layered node-link "river" with a details-on-demand sidebar. Pure projection of the existing trace graph (DEC-5, no new engine, DEC-10): a breadth-first **transitive walk under the `EDGE_ORIENTATION` map** (`src/traceView.ts`, note 05 §4) — the one-hop menu's orientation reading applied depth by depth — cycle-guarded, dead ends surfaced (`known: false`, G3), symmetric `contests` edges shown on both flanks (once each, never looped). Reached from the SPEC-16 one-hop trace menu's "View full graph →" link as a **full-screen overlay, not a sixth tab** (DEC-32's four role tabs and DEC-36's single shared-surface tab stand; Observer sub-mode is the stated fallback). The one genuinely-new invariant obligation is a **truncation-honesty rule** — the G4 no-silent-drop discipline lifted to a view: both the breadth cap ("+ K more" per layer) and the depth cap ("⋯ deeper chains continue beyond depth N") are rendered, never silent. No schema change; no oracle, verdict, or coverage-row change; read-only, moves no thesis. **Process note:** the slice was prototyped mock-first and built (`src/depGraph.ts` + `components/{depGraphRiver,depGraphSidebar}.ts`, wired into the shell overlay, `tests/depGraph.test.ts`) *ahead* of its research note — an inversion of DEC-11's research-first order, now closed by note 13, which decides the design on its merits and audits the build against it (§7). Candidate only — recorded here per DEC-2; neither the build nor note 13 may be cited as authority until ratified.

30. **The decision-history replay surface (SPEC-26)** *(flagged 2026-07-16; review §6.2/M5 + addendum §10 slice S-G; decided technically by research note `14-replay.md` under the DEC-11 gate)*. DEC-32/DEC-36-class shell growth: a **decision-time scrubber** over the existing delta log — an app-layer `HistoryCursor` (`{seq, mode: live|replay}`) whose view at cursor `n` is the **event-sourcing fold** `state(n) = fold(∅, deltas[1..n])`, implemented as the immutable content-addressed store (DEC-21) filtered by a *derived, non-authoritative* `created_seq ≤ n` index and re-driven through the same `snapshot()` pipeline — byte-equal to a fresh replay by G1 (no rollback, no stored intermediate state). Refusals, staleness flags, stamps and verdicts **re-derive** (they are functions of `state(n)`, not replay artefacts): the K12 contest banner re-renders at its seq exactly as it rendered live, and a refused write is a recorded cursor position with unchanged state (an attempt is part of the record, never elided — depends on DEC-5 publishing a delta for refused writes; any gap is a coverage defect to log, never synthesised). Cursor transitions re-fire the glow under the live value-keyed rule (DEC-34); replay disables writes, is visibly marked, and returns cleanly to the live head, with new live deltas surfaced as an "M new" count (never a silent jump). Distinct from — and composed with — SPEC-19's scenario-clock scrub (world time vs decision time; two separate controls, never one conflated slider). Checkpointing is decided as a rule (cache `snapshot()` at compile boundaries, fully re-derivable and discardable, the log stays the only authority) but unused in v1 at Meridian scale. No schema change, no seam change (the delta log already crosses the seam). Candidate only — recorded here per DEC-2; SPEC-26 may not be cited as authority.

31. **The recursive-trace tooltip affordance (SPEC-26)** *(flagged 2026-07-16; review §6.2/M7 + addendum §10 slice S-G, the shallow end of concept §6.29 / issue #24; decided technically by research note `14-replay.md` under the DEC-11 gate)*. DEC-34-class growth: the SPEC-16 one-hop "informs / influenced-by" menu (`src/traceView.ts`) made **recursively expandable in place to a stated depth cap of 3** — note 13's transitive walk under the `EDGE_ORIENTATION` map, depth-capped, the same traversal as the full graph, **agreeing byte-for-byte with `TraceStore.walk`** for the same origin (one semantics, no parallel walker). Each hop renders its real relationship — the `edge_type` in the trace graph's own vocabulary plus a *fixed* operation gloss for computation edges (`scored_from` = the interval evaluation over channel reads, `compiled_into` = the band materialisation), never an invented "why"; dead ends render as dead ends **at every depth** (`known: false`, G3); symmetric `contests` edges show on both flanks (once each, cycle-guarded). The cap is a stated constant (informed by the ~3–4 working-memory chunk ceiling, CK3's practitioner precedent), not adaptive. At the cap an **honest, counted remainder** ("N more — open full trace") routes to the existing trace drawer and, once issue #24 lands, to note 13's full-screen graph view — the G4 no-silent-drop discipline at the tooltip layer, never a silent truncation. No schema change; read-only projection of the existing trace graph. Candidate only — recorded here per DEC-2; SPEC-26 may not be cited as authority.

32. **Narratives-as-scrub-paths — the runner drives the scrubber (SPEC-26)** *(flagged 2026-07-16; review §6.2 + addendum §10 slice S-G; decided technically by research note `14-replay.md` under the DEC-11 gate)*. A decision touching **DEC-23's demo-moment machinery**: each Stage-7 narrative is re-expressed as an ordered list of `(seq, tab, note)` **scrub-path waypoints** over one canonical recorded history, and the narrative runner *drives the decision-history scrubber* (§6.30) — set cursor to the waypoint's seq, raise its tab, show its note — **retiring the bespoke beat/action mechanism** (`NarrativeBeat.action`, the imperative `resolve`/`edit-k8` live mutations in `shell.ts`) in the same change. The honesty gain: a beat can only point at a recorded seq, so a narrative can render only what the record actually contains — narrative and record can no longer drift. There is **one canonical heartbeat history** (the walkthrough seq run, extended to cover every act any narrative references) with each narrative a curated ordering of waypoints into it; a state a narrative names but the record lacks is a coverage gap to close in the record, never a bespoke action smuggled back. The **SPEC-17 presenter contract is preserved verbatim** — navigation, notes panel, doctrinal quotes, centrepiece marker, wall-projection mode, and the banded-honesty presenter-note guard (`tests/narratives.test.ts`, note 09 §3/§4, now run against each waypoint's `note`). DEC-23's requirement (each stage exit paired with an observable twin and a scripted demo) is unchanged — only its plumbing: the runner assembles the banked moments *from the record* rather than from a parallel beat list. No schema change. Candidate only — recorded here per DEC-2; SPEC-26 may not be cited as authority.

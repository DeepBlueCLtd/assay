...
17. **[CANDIDATE — flagged for next register batch]** **Propagation feedback (the "glow") as G6 made visible.** When an edit on one surface changes data another surface depends on, the affected tab and the affected downstream components **glow** for a fixed window — derived, not decorative: a panel glows **iff a `content_hash` in its dependency set changed** on recompute, read straight off the trace graph (the same `TraceEdge` set and per-`edge_type` orientation map `05-surfaces.md` defines for the trace menu). The glow is therefore the standing **propagation-honesty invariant (G6)** rendered as an operator-visible affordance — it must never *under*-report (every changed downstream glows) any more than the scorer may silently drop a consequence. A companion one-hop, expandable **"informs / influenced by" context menu** on any item exposes the same graph on demand (the focused transitive dependency view is noted as horizon-only, tracked as an issue). SPEC-16 builds against this candidate under DEC-2 delegated authority, pending batch ratification.

...
22. ~~Compile-overlay precedence — excursion-layer overrides beat base-knowledge overrides (SPEC-20)~~ — **ratified 2026-07-16 as ASSAY-DEC-40** (batch 8): channel materialisation resolves overlapping `RegionOverride`s by layer before geometry — an excursion override (DEC-8) beats a base-knowledge override on the same cell/step; the layer is derived (`override.source === world.scenario`), never stored (no schema change); `ENGINE_VERSION` → 0.2.0. Decided by `02-compile.md` §6; built under SPEC-20.

23. ~~`jipoe_step` on `KnowledgeObject` — knowledge names its doctrinal origin (SPEC-21)~~ — **ratified 2026-07-16 as ASSAY-DEC-41** (batch 8): a `JipoeStep` enum + optional `jipoe_step` slot (schema change, touches DEC-6), the *originating* step singular by design, a warning-level `missing_jipoe_step` lint (`observed` not exempt), the provenance chip rendering the step in words. Decided by the `01-knowledge.md` amendment; built under SPEC-21.

24. ~~Attention ordering — the two licensed weight-consuming behaviours, and nothing else (SPEC-22)~~ — **ratified 2026-07-16 as ASSAY-DEC-42** (batch 8): the §9 firewall sharpened to exactly two behaviours (the scenario strip's interval-order rendering; the S1 queue's equal-standing tie-break), the set declared closed, no scalar sort key anywhere, "attention only" machine-checked. Decided by `11-attention.md`; built under SPEC-22.

25. ~~Provenance on `ExpectedAnswer` — the event matrix carries a chip (SPEC-23)~~ — **ratified 2026-07-16 as ASSAY-DEC-43** (batch 8): an optional `provenance` slot (the whole `Provenance` shape) on `ExpectedAnswer` (schema change, touches DEC-18/DEC-9), a warning-level `missing_expected_answer_provenance` lint, the chip wherever an expected band renders. Decided by the `08-analysis.md` §7.3 amendment; built under SPEC-23.

26. ~~Operative-pair conditioning — discrimination leads with the pairs the live decision turns on (SPEC-23)~~ — **ratified 2026-07-16 as ASSAY-DEC-44** (batch 8): operative pairs derived from verdict divergence in the SPEC-10 tensor (K14 firewalled), the three-way separation classification (disjoint/partial/nested with nested excluded from could-discriminate emphasis), no metric change (v1 separations byte-identical). Decided by the `08-analysis.md` §7.1–§7.2 amendment; built under SPEC-23.

27. ~~The decision-point derivation rule — the DSM's semantics (SPEC-24)~~ — **ratified 2026-07-16 as ASSAY-DEC-45** (batch 8): the two-class DP predicate (scenario-divergent or margin-class; uniform robust/violated non-DPs), the commit step from the metric's own reads (world-decided for `state` metrics; stated absence for margin-class rows), LTIOV = commit_step − lead (lead 0, stated) with the three-state answerable-in-time predicate, no urgency scalar (DEC-19). Decided by `12-decision-support.md`; built under SPEC-24.

28. ~~The DSM surface and seam addition — `POST /analyse/decision-support` (SPEC-24)~~ — **ratified 2026-07-16 as ASSAY-DEC-46** (batch 8): a thin-orchestration analysis endpoint returning stamped, `cited_in`-edged `DecisionPointRow`s, surfaced as an S2-adjacent "Decisions in time" panel on the commander tab (not a sixth tab; DEC-32/DEC-36 stand), no tasking affordance, no schema change (movement types only). Built under SPEC-24.

29. ~~The focused transitive dependency-graph surface (issue #24)~~ — **ratified 2026-07-16 as ASSAY-DEC-47** (batch 8): a full-screen, on-demand transitive walk under the `EDGE_ORIENTATION` map (the one-hop reading applied depth by depth; DEC-5/DEC-10, read-only), reached as an overlay off the trace menu (not a tab), with a truncation-honesty rule (breadth and depth caps rendered, never silent). The full-screen big brother of DEC-38's recursive tooltip; decided by `13-dependency-graph.md` (which also audited the mock-first DEC-11 inversion — a findings-ledger process observation); built under issue #24.

30. **The consequence-preview interaction class — "compute what a write would change, show it, commit nothing" (SPEC-25)** *(flagged 2026-07-16; review §3.1/§6.2/M4 + addendum slice S-F; decided technically by research note `14-legibility.md` §2 under the DEC-11 gate)*. A new kind of *read* the constitution should bless: arming a mutating act (resolve, supersede, waiver grant, plan edit) computes its real verdict-matrix consequence over a **byte-for-byte shadow of the committed store**, run through the same service methods and the same firewall as a commit, and shows it as an unmistakably-uncommitted ghost diff — then commits exactly that or cancels to a byte-identical state. Its binding honesty rule is **previewed = computed, never estimated** (the *Into the Breach* telegraphed-consequence contract applied to knowledge work — DEC-10 forbids a second estimating engine): the previewed diff **equals** the post-commit glow set element-for-element (G1 guarantees it, a test asserts it, DEC-34 carries for free); the shadow persists nothing, emits no delta, mutates no stamp; a **dishonest armed edit previews its refusal** (G2/G5 apply to hypotheticals with full force). The subject is an app-layer `PreviewState {armed_act, shadow_result, ghost_diff}` — never stored, never stamped; the client-side fork needs no seam change (immutability makes the fork cheap — SPEC-01), a seam-level `POST /preview` contract left as a *further* candidate if a second consumer appears. Candidate only — recorded here per DEC-2; SPEC-25 may not be cited as authority.

31. **Per-role action menus as a standing shell element — C2 role separation made legible (SPEC-25)** *(flagged 2026-07-16; review §3.1/M10 + addendum slice S-F)*. A DEC-32-class shell-structure finding, recorded rather than asserted: each role tab surfaces its **legal write verbs** from a single source-of-truth role→verb map (J-2: create/contest/resolve/supersede/collect-mark; planner: compile/generate-handful/relax/score; commander: select/waive; observer: none — read and trace only; the Spatial · COA surface names its authoring inputs and marks itself a cross-role *surface*, not a role), each annotated by how it is exercised here — live-actionable, pipeline-automatic, or deferred-and-labelled (never a dead button faking a write, DEC-4; `select`/`POST /select` is named-but-unbuilt). The note's finding is that this **refines DEC-32/33, it does not fork them** — the menu adds no write surface, only re-presents existing gated affordances and documents the legal set (menus reorganise, never restrict; observer read-only is the load-bearing assertion). Register-neutral **iff** pure re-arrangement (the note holds it is); flagged conservatively because "a per-role action menu as a standing shell element" is a shell-structure claim of DEC-32's class. Candidate only — recorded here per DEC-2; SPEC-25 may not be cited as authority.

32. ~~The decision-history replay surface (SPEC-26)~~ — **ratified 2026-07-16 as ASSAY-DEC-37** (batch 7): a decision-time scrubber over the delta log whose view at cursor `n` is the event-sourcing fold `state(n) = fold(deltas[1..n])`, implemented as the immutable store (DEC-21) filtered by a derived, non-authoritative `created_seq` index (a filter, never a rollback; byte-equal by G1); refusals re-derive, the glow (DEC-34) re-fires on cursor transitions, decision-time replay composes with but is distinct from SPEC-19's world-time scrub; no schema/seam change. Decided by `15-replay.md`; built under SPEC-26.

33. ~~The recursive-trace tooltip affordance (SPEC-26)~~ — **ratified 2026-07-16 as ASSAY-DEC-38** (batch 7): note 13's transitive walk depth-capped at a stated 3, agreeing byte-for-byte with `TraceStore.walk`, each hop labelled from `edge_type`, dead ends at every depth (G3), an honest counted remainder handing off to the trace drawer / issue #24 (its shallow end). Decided by `15-replay.md` §5; built under SPEC-26.

34. ~~Narratives-as-scrub-paths — the runner drives the scrubber (SPEC-26)~~ — **ratified 2026-07-16 as ASSAY-DEC-39** (batch 7): each Stage-7 narrative becomes `(seq, tab, note)` waypoints driving the scrubber, retiring the bespoke beat/action mechanism while preserving the SPEC-17 presenter contract verbatim; DEC-23's demo-moment requirement unchanged, only its plumbing (assembled from the record, not a parallel beat list). Decided by `15-replay.md` §4; built under SPEC-26.

35. **The recursive-trace tooltip's breadth cap — width truncation is as honest as depth truncation (SPEC-26 US3 rescope)** *(flagged 2026-07-17; `15-replay.md` §5 rescope amendment)*. DEC-38 fixed the tooltip's *depth* cap at 3 but was silent on *width*. The US3 build (rescoped to integrate with the now-shipped DEC-47 graph, not pre-figure it) found that a single downstream hop from a knowledge object fans into every verdict (the world it compiles into is `scored_from` by all of them) — Chromium verification produced a ~6000px "tooltip." The refinement is not a new idea but the DEC-47 view's *own* per-layer breadth truncation lifted to the tooltip layer: each hop shows a handful of neighbours (a stated constant, `RECURSIVE_TRACE_BREADTH_CAP = 6`) and a counted "+N more — open full trace" that opens the shipped overlay focused on that hop — the same G4 no-silent-drop remainder the depth cap already renders, and the wide reading explicitly deferred to the surface built for it. No traversal-semantics change (still the shared `neighbours`/EDGE_ORIENTATION reading), no schema change, no seam change. Candidate only — recorded here per DEC-2; extends DEC-38's rendering, does not assert. The DEC-38 handoff wording ("the trace drawer / issue #24") is likewise realised now that #24 has shipped: the remainder opens the DEC-47 overlay, there is no separate drawer.

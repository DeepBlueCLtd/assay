...
Status: canonical · v0.4 · 2026-07-18 · retitled from `assay-scaffold.md` per §4 (batch 2); §6 candidates 6–11 ratified as ASSAY-DEC-22…27 (batch 3); §6 candidates 12–18 ratified as ASSAY-DEC-28…34 (batch 4); §6.20 as ASSAY-DEC-35 (batch 5); §6.21 as ASSAY-DEC-36 (batch 6); §6.32–6.34 as ASSAY-DEC-37…39 (batch 7); §6.22–6.29 as ASSAY-DEC-40…47 (batch 8); §6.30/§6.31/§6.35 as ASSAY-DEC-48…50 (batch 9). Candidate queue now empty but for horizon §6.19 (issue #43).
...
30. ~~The consequence-preview interaction class — "compute what a write would change, show it, commit nothing" (SPEC-25)~~ — **ratified 2026-07-18 as ASSAY-DEC-48** (batch 9): arming a mutating act computes its real verdict-matrix consequence over a byte-for-byte shadow of the committed store (`AppState.fork`), run through the same service methods and firewall as a commit, shown as an uncommitted ghost diff; the binding rule is **previewed = computed, never estimated** — the previewed diff equals the post-commit glow set element-for-element (G1, `src/preview.ts`), the shadow persists nothing and mutates no stamp, a dishonest armed act previews its refusal (G2/G5); an app-layer `PreviewState`, no seam change (a `POST /preview` contract a further candidate). Decided by `14-legibility.md` §2; built under SPEC-25.

31. ~~Per-role action menus as a standing shell element — C2 role separation made legible (SPEC-25)~~ — **ratified 2026-07-18 as ASSAY-DEC-49** (batch 9): each role tab surfaces its legal write verbs from one source-of-truth `ROLE_VERBS` map (observer: none — read and trace only), each annotated live-actionable / runs-on-recompute / not-yet-built (never a dead button, DEC-4); the menus **refine DEC-32/33, they do not fork them** — no new write surface, only re-presented gated affordances and a documented legal set (menus reorganise, never restrict; observer read-only load-bearing); the Spatial · COA surface stays a cross-role *surface*, not a role. Decided by `14-legibility.md`; built under SPEC-25.
...
35. ~~The recursive-trace tooltip's breadth cap — width truncation is as honest as depth truncation (SPEC-26 US3 rescope)~~ — **ratified 2026-07-18 as ASSAY-DEC-50** (batch 9): DEC-38's depth cap (3) extended to width — each hop shows a stated constant `RECURSIVE_TRACE_BREADTH_CAP = 6` neighbours and a counted "+N more — open full trace" opening the shipped DEC-47 overlay focused on that hop (the same G4 no-silent-drop remainder, the wide reading deferred to the full-screen surface); no traversal-semantics change (the shared `neighbours`/`EDGE_ORIENTATION` reading, machine-checked against `buildDepGraph`), no schema/seam change; the DEC-38 "trace drawer / issue #24" handoff realised — the remainder opens the DEC-47 overlay. Decided by the `15-replay.md` §5 rescope amendment; built under SPEC-26 US3.

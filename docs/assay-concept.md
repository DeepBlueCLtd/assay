...
15. **[CANDIDATE — flagged for next register batch]** **System-flow infographic — behavioural decisions.** The proposal `docs/assay-flow-infographic-spec.md` (+ companion `assay-flow-infographic-wireframes.html`) grows the S4 Bridge into a stakeholder-facing explainer of the object/connector/gate flow, embedded twice from one component (in-app S4 "systems-map" mode + public Pages explainer). Most of it is a pure projection of existing seam behaviour and needs no decision; four behaviours do, and are recorded here rather than asserted in the spec (DEC-2): **(a)** *live/auto recompute in this view, attribution-visible* — the fan-out is instant (a `/analyse/staleness` read) but the recompute that turns flags into verdicts fires automatically **while remaining stamped, delta-logged, and gate-policed** (badge flips, a delta lands, the gate pulses); this differs deliberately from S2's "recompile when ready" (ui-design §3.4.4) and so is a candidate, not a silent override of it — the walkthrough's "nothing recomputes silently" is preserved because attribution is mandatory, only the *trigger* is automated. **(b)** *Viewer-driven writes from a mock operator* in the sandbox — all four surfaces are currently read projections (S4 especially watches, never writes), so a bounded viewer-write palette (toggle R1/R2/R3/R3m, contest/resolve K12, supersede K5→K9, grant/withhold W-1) against the real mock seam is a surface-behaviour decision. **(c)** *The map/geospatial panel* (ui-design §6.1 open question) — the flow view is a natural first home for the grid/causeway/strait; pulling it in is a live decision. **(d)** *A truly-silent "you-are-the-optimiser" teaching toggle* — deferred, to be reacted to by an SME (DEC-27 checkpoint) before it hardens. The interactive view is a **build slice** (SPEC-14 site/gallery neighbourhood or its own SPEC-##); the explainer framing around it is **communications authoring** under item 14's candidate taxonomy. **Proceeding under delegated authority** (decided 2026-07-13, the §6.12/6.13 pattern): the build slice **may implement against these candidates and ratify on landing** per DEC-2's established escape hatch — building against a flagged candidate is permitted, only *asserting* it as settled is not. Delegated authority is **scoped to the register gate (DEC-2) and does not waive research-first (DEC-11)**: a `docs/research/` note precedes any implementation, as notes `01`/`02` preceded the schema changes they carried; a missing note is the gate, not a formality. *Flagged candidate — building under delegated authority; not yet ratified.* **Landed 2026-07-13** — the flow view is **built** (SPEC-14 flow-view sub-slice: `src/flow.ts` + `src/flowPage.ts` + `scripts/build-flow.ts`; `specs/014-flow-view/`; research note `07-flow-view.md`; `tests/flow.test.ts` covering AS-1…AS-11). Candidates **(a)** auto-recompute (attribution-visible: stamp flip + delta row + gate pulse; S2's "recompile when ready" left unchanged), **(b)** the bounded viewer-write sandbox palette (toggle R1/R2/R3/R3m · contest/resolve K12 · supersede K5→K9 · grant/withhold W-1), and **(c)** the map/geospatial panel (ui-design §6.1) — a Meridian schematic rendered from the real `VignetteConfig` region geometry with the current world's threat/mobility channel overrides overlaid live (a projection of the compiled world, never a hand-drawn claim; the R1/R2/R3/R3m excursion is visibly different on the map) — are all **implemented and demonstrated** against the real seam; only **(d)** the truly-silent "you-are-the-optimiser" toggle stays **deferred** for an SME reaction. Per DEC-2's escape hatch the build did not *assert* these as settled: **the ratifying entries (a)/(b)/(c), and the recorded deferral (d), are owed in the next register batch — ratify-after debt** (the §6.12/6.13 pattern). Nothing here is asserted as decided.

16. **[CANDIDATE — flagged for next register batch]** **Surface shell = literal tabs in one live single-page app over a shared client store.** Research note `05-surfaces.md` (the front-end DEC-11 gate) decides the long-open shell question — concept §6 item 4 ("literal tabs in one SPA vs routed micro-frontends over the shared store") and `assay-ui-design.md` §6 item 2 ("literal tabs … vs role-switch with distinct chrome") — in favour of **four literal role tabs (J-2 / J-3-5 planner / Commander / Observer) in one page, over one in-browser object+trace store and one instance of the real services**. The "different roles" illusion the tabs weaken is recovered honestly by the heartbeat loop being *visible* (item 18): an act on one tab visibly propagates to the others. Resolves items 4 and (ui-design) 2. SPEC-16 builds the shell against this candidate under DEC-2 delegated authority, pending batch ratification.

17. **[CANDIDATE — flagged for next register batch]** **Live in-browser pipeline + editable surfaces — the shell/pure-component split preserves SPEC-14 extractability.** The published app runs the *actual* `Knowledge/Compile/Score/Handful/Relax` services **client-side** (the hashing already targets `globalThis.crypto.subtle` for exactly one browser+Node code path — `canonical.ts`), and operator edits are a first-class **authoring act** (bearing on item 5, "hand-authored JSON fixtures vs a light authoring surface"): the surfaces become the light authoring surface, seeded from the fixtures. This reopens the **SPEC-14 extractability constraint** ("components depend only on the generated LinkML types, never on app state or services" — delivery plan) — resolved, not waived, by a **shell / pure-component split**: the eight `src/components/*` renderers stay pure (types-in, HTML-string-out); *all* state, events, recompute and propagation live in a new `src/app/` shell layer that wraps them. Every edit is gated by the same honesty machinery the services already enforce (`encoding.ts`, `lint.ts`, contested-knowledge refusal) — a dishonest edit returns a first-class `Refusal`/`waiver_required`, never a silent fabricated result, and no assessed scalar is rendered unbanded under any edit (G2). SPEC-16 builds against this candidate under DEC-2 delegated authority, pending batch ratification.

18. **[CANDIDATE — flagged for next register batch]** **Propagation feedback (the "glow") as G6 made visible.** When an edit on one surface changes data another surface depends on, the affected tab and the affected downstream components **glow** for a fixed window — derived, not decorative: a unit glows **iff its displayed value changed** on recompute (each pure component stamps a `data-glow-id` + value `data-glow-sig`; the shell diffs by value), so the glow is row/cell-level and never over-reports a mere re-derivation. The glow is therefore the standing **propagation-honesty invariant (G6)** rendered as an operator-visible affordance — it must never *under*-report (every changed downstream glows) any more than the scorer may silently drop a consequence. A companion one-hop, expandable **"informs / influenced by" context menu** on any item exposes the trace graph on demand (the focused transitive dependency view is noted as horizon-only, tracked as an issue). SPEC-16 builds against this candidate under DEC-2 delegated authority, pending batch ratification.

...
29. **The focused transitive dependency view — a drill-in provenance surface (issue #24 / SPEC-27)** *(flagged 2026-07-16; requested by the user during the SPEC-16 review; decided technically by research note `14-dependency-view.md` under the DEC-11 gate)*. A **presentation** decision, no schema and no semantics change: pick any item (K/C/R/P) and see its entire upstream/downstream chains at once, as a horizontal river (upstream → focus → downstream) with a click-driven detail sidebar, opened as a **full-screen overlay** from the existing one-hop trace menu's "View full graph →" link — **not a sixth tab** (DEC-32's four role tabs stand; DEC-36's Spatial surface is the one ratified exception, and a drill-in lens you point at any item is neither a role nor a standing surface). The one real technical decision is that the transitive walk goes by **orientation-resolved direction, never raw hash direction** — `EDGE_ORIENTATION` (note 05 §4) applied at every hop, BFS by orientation-distance — because `TraceStore.walk` is orientation-blind and a naive forward walk from a K stops at the world; the view adds a second orientation-aware wrapper beside the menu and staleness, no new graph mechanics. Scale is bounded honestly: a depth cap that stops with a non-empty frontier **says so** (`upstreamTruncated`/`downstreamTruncated`, a stated river end-cap), a wide layer shows `+ N more` with a full type summary, dead-ends render as dead-ends (G3) — G3/G4 applied to the view, **no new invariant** (contrast the horizon note's genuinely-new "no silent COA-family drop" for *generation*, issue #43 §2.5 — this view generates nothing). Read-only: re-centring on a clicked node is selection, not computation (no recompute, no glow, DEC-36(c)'s scrub line); no scalar, no band collapse (G2/G6). The **antithesis** of thesis G's false-precision risk (concept §1; knowledge-model §12) — it renders only edges a service wrote at compute time (constitution III), never inferred interdependencies. Reuses the existing `TraceStore` + `EDGE_ORIENTATION` (no new engine, DEC-10). The code was authored ahead against issue #24 (before this note existed) and is now retro-anchored to the DEC-11 note and formalised as SPEC-27; the note's one correction to the shipped code — surfacing depth-truncation instead of silently dropping it — is folded in. Candidate only — recorded here per DEC-2; SPEC-27 may not be cited as authority.

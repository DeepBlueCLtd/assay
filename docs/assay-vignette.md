...
| `K10` | Will the garrison capitulate by D+5? | **hard_constraint claimed — REFUSED** | assessed (émigré political network) · low | Refused `encoding_violation` at POST /knowledge: an assessment with no waiver claiming certainty about will-to-fight; withdrawn (`retired`) | **The refusal exhibit (Stage-1 exit)**; the honest system declining laundered judgement |

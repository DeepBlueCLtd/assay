...
2. ~~Surface shell: literal tabs in one SPA vs role-switch with distinct chrome per surface.~~ — **decided: literal tabs in one SPA over a shared client store** (research note `05-surfaces.md`; concept §6 candidate 14, flagged for the next register batch). The "different roles" illusion tabs weaken is recovered honestly by the heartbeat loop being *visible* — an edit on one tab glows the tabs and components it touches (G6 made visible; concept §6 candidate 16). Built in SPEC-16 (`src/app/*`); the live surface is published at `assets/app/`.

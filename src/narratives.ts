 ...
 * unified onto SPEC-26 scrub paths (DEC-39, note 15 §4).
 *
 * A narrative is no longer a parallel beat-state mechanism: each beat is a
 * WAYPOINT — a `(seq, tab, presenterNote)` triple into the ONE canonical
 * recorded heartbeat (`AppState.seedCanonical`). The runner drives the
 * decision-history cursor to `waypoint.seq` and raises its tab; the surface
 * state is therefore the scrub reconstruction at that seq, never bespoke state
 * left by a scripted mutation. A narrative can render ONLY what the record
 * contains — narrative and record can no longer drift (note §4). The SPEC-17
 * presenter contract (nav, notes, doctrinal quotes, centrepiece, wall mode, the
 * banded-honesty note guard) is preserved verbatim; the bespoke `action`
 * mechanism is gone.
 *
...
export interface NarrativeBeat {
  /** The decision-history cursor this beat scrubs to (into the canonical
   *  heartbeat; the beat→seq table in tests/narratives.test.ts pins it). */
  seq: number;
  tab: TabId;
  title: string;
...
/** A narrative IS a scrub path — an ordered list of waypoints into the one
 *  canonical recorded history (DEC-39). Alias for legibility at call sites. */
export type ScrubPath = NarrativeBeat[];
  ...
  // ---- Bridge narrative ----
  {
    ...
    beats: [
      ...
      },
      {
        ...
        presenterNote:
          'On the "J-2 · knowledge base" panel, the J-2 has superseded K8 "days to repair the battery fire-control radar" — a shifted band. Scrubbing here (seq 21 of the record) shows the supersede: a new version, a supersedes edge, and a delta, all in the "Observer · the seam, watched" panel. The glow tells you who is affected: the J-2 workbench originated the change, but the Planner and Commander surfaces move (their content depends on this knowledge). Nothing was scripted — the beat scrubs to the point in the record where the edit actually landed.',
      },
      {
        seq: 21,
        tab: 'observer',
        ...
        presenterNote:
          'Back on Observer / bridge, the "Observer · staleness flags" panel shows exactly which downstream artefacts the supersession invalidated — the storm-channel-dependent verdicts, and nothing else (thesis F). The flags arrived through the trace graph, not through a meeting. The planner sees "recompile when ready"; the commander sees the affected cards marked stale. One object model, one store, one trace graph — the bridge is the seam itself.',
      },
      {
        seq: 21,
        tab: 'observer',
  ...
  // ---- REMIT narrative ----
  {
    ...
    beats: [
      ...
      },
      {
        ...
        presenterNote:
          'On the "J-2 · knowledge base" panel, the assessed bands and provenance chips are visible on every entry. The encoding firewall (G2) and confidence lint (DEC-16) are not rules the operator follows — they are constraints the system enforces. This seq IS a refused attempt — an edit that tried to make an assumption a hard constraint. The "Observer · the seam, watched" panel records it as a `refused` delta: the attempt is part of the record, nothing was written, the state is unchanged. The refusal is first-class, never an error toast.',
      },
      {
        seq: 20,
        tab: 'planner',
      ...
      },
      {
        ...
        presenterNote:
          'At the head of the record, the "Observer · the seam, watched" panel shows the whole sequence the scrub path just walked: create → supersede → contest → a refused attempt → resolve → recompile → the K8 edit, each stamped, each typed, each traced. The seam is one shared store, walked end-to-end — and the narrative itself is a curated path into that record, so it can show only what actually happened. That is the claim: the architecture holds under the invariants the register imposes (G1–G6), and every joint is examinable.',
      },
    ],
  },
];

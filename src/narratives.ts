    ...
    beats: [
      {
        ...
        presenterNote:
          'This is the S1 table — the knowledge base. Every value is a range, not a number: K8\'s band is 30 to 60, never a single midpoint. The provenance chips alongside each entry show source class and confidence — they are welded on, never optional. Notice K8: it is single-source, held under waiver W-1.',
      },
      {
        tab: 'j2',
        title: 'Sensitivity ranking — which beliefs are load-bearing',
        presenterNote:
          'The sensitivity panel (thesis E) ranks which beliefs are load-bearing. K8 tops the ranking: perturbing its band edge changes commitment verdicts on C3 and C4 — more than any other knowledge object. It is also single-source (ICD 203 flag). The system identifies that K8 is load-bearing; the J-2 decides what to do about it.',
        doctrinalQuote:
    ...
    beats: [
      {
        ...
        presenterNote:
          'This is the S2 honest matrix — plans as rows, commitments as columns, each cell a four-stop verdict chip: robust, marginal, tight, violated. There is no total row and no weighted sum — the pattern of verdicts IS the comparison, and the commander weighs it, not the machine. Under the BASE scenario, P1 holds C1 and C2 robust.',
      },
      {
        tab: 'commander',
        title: 'Scenario robustness — the favourite under stress (thesis C)',
        presenterNote:
          'The Commander tab\'s scenario strip lets you toggle R2 (Strait Denial). P1\'s C1 and C2 drop from robust to violated — the strait-early plan dies under mines. P2 holds. P1 is robust under BASE but violated under R2; the verdict is a category, not a rank. The collapse markers (▼) show exactly which cells degraded.',
        doctrinalQuote:
  ...
  // ---- Commander narrative ----
  {
    ...
    beats: [
      ...
      },
      {
        ...
        presenterNote:
          'Clicking a verdict chip on any card opens the trace menu: one-hop informs (upstream knowledge) and influences (downstream artefacts). Every element traces back to named knowledge with named owners (G3). The commander sees not just the sacrifice but WHY — which assessment drives it, who owns that assessment, and how strongly it is held.',
      },
      {
        tab: 'planner',
        title: 'The honest matrix — the trade the cards summarise',
        presenterNote:
          'On S2, the same verdicts that fill the cards are the cells of the matrix. The card is a summary; the matrix is the detail. Neither has a total row. P2 holds C1 robust, C2 marginal, C3 tight, C4 violated — never "P2 scores 2.5 out of 4."',
      },
      {
        tab: 'commander',
        title: 'Centrepiece — the sacrifice is stated, never silent',
        presenterNote:
          'Back on S3, the three cards are the deliverable: each sacrifice named, each tie-break stated, each traced to evidence. The commander\'s decision is supported by argument, not by arithmetic. This is the Stage-4 exit: least-worst, never silence.',
      },
    ],
  },
    ...
    beats: [
      {
        ...
        presenterNote:
          'The Observer tab (S4) shows the delta feed — every knowledge write as a stamped, sequenced entry. This IS the bridge: every act by every role is visible here, typed and traceable. No email, no annex, no verbal caveat that evaporates.',
      },
      {
        tab: 'j2',
        title: 'The J-2 edits a band — supersession in action',
        presenterNote:
          'On S1, the edit control lets you supersede K1\'s band — change from the current range to a shifted range. The supersede writes a new version, a supersedes edge, and a delta — all visible in the Observer feed. The glow tells you who is affected: the J-2 tab does not glow (it originated the change), but the Planner and Commander tabs do (their content depends on this knowledge).',
        action: 'edit-k8',
      },
      {
        tab: 'observer',
    ...
    beats: [
      {
        ...
        presenterNote:
          'The Observer tab (S4) shows the seam contract in action: every write publishes exactly one stamped delta (seam §10). Objects are immutable and content-addressed (DEC-21); edges are written at write time, never reconstructed (constitution III). This is the architecture: does it hold under the banded-honesty invariant?',
      },
      {
        tab: 'j2',
        title: 'Knowledge capture — banded honesty in practice',
        presenterNote:
          'On S1, the assessed bands and provenance chips are visible on every entry. The encoding firewall (G2) and confidence lint (DEC-16) are not rules the operator follows — they are constraints the system enforces. Try a dishonest edit (e.g. a band narrower than the source class warrants): the system refuses with a named reason. The refusal is first-class, never an error toast.',
      },
      {
        tab: 'planner',
        title: 'Compile → score → handful — the pipeline walks forward',
        presenterNote:
          'On S2, the compiled world (sparse channels), the handful strip, and the honest matrix show the pipeline: compile → score → organise, each step writing trace edges. The handful is generated (seeded fan-out), not authored — the organiser uses banded ε-non-domination (DEC-19), never a scalar ranking.',
      },
      {
        tab: 'commander',
        title: 'Relaxation — least-worst as a design decision',
        presenterNote:
          'On S3 (Commander tab), the relaxation report embodies the design decision ASSAY re-derived: ordinal tiers, stated tie-breaks, no weights (DEC-19). REMIT\'s DEC-22 uses priority-weighted soft-commitment penalties; ASSAY shows the same mechanism works without weights — FIND-2 in the findings ledger. The cards speak command language because the commitment label is authored in the vignette, not generated.',
      },
      {
        tab: 'observer',
        title: 'Centrepiece — the full heartbeat, end-to-end',
        presenterNote:
          'Back on Observer, resolving K12 and then editing a band triggers the full pipeline. The delta feed shows the sequence: create → supersede → contest → resolve → recompile → score → relax, each stamped, each typed, each traced. The seam is one shared store, walked end-to-end. That is the claim: the architecture holds under the invariants the register imposes (G1–G6), and every joint is examinable.',
        action: 'resolve',
      },
    ],
  },

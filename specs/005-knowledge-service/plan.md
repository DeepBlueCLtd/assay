...
**Principle III — Traceability Terminates in Named Knowledge.** PASS. `supersedes`/`contests`/`resolves` edges are written at the moment of the act by the service performing it (FR-006/007/008); the licensing waiver is recorded inline on the object and its `waives` edge is written at compile time when the constraint is built (FR-004; seam §4); exposure returns a complete forward chain and an object driving nothing returns empty-but-complete, never a dead end (FR-009). Encoding discipline (the firewall) is enforced here as part of this principle. **(G3, G5)**

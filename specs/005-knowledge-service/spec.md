...
A J-2 analyst tries to save an assessment as though it were a hard fact — an émigré political-network judgement that the garrison *will* capitulate by D+5 (K10), claimed as a `hard_constraint` with no licensing waiver. The system refuses the save at the moment it is attempted, names exactly what is wrong, and stores nothing. This is the demonstrator's signature moment: **"the system declines laundered judgement."**
...
1. **Given** an assessment (`source_class = assessed`) claiming `hard_constraint` with no recorded waiver, **When** a J-2 analyst attempts to save it, **Then** the write is refused with `encoding_violation`, the offending ref is K10, and nothing is stored (DEC-14, DEC-6; G2 upstream defence).
...
1. **Given** an assessed, single-source value claiming `hard_constraint` **with** waiver W-1, **When** it is saved, **Then** it is accepted and a `waives` relationship is recorded against the licensing waiver (DEC-14; constitution III).

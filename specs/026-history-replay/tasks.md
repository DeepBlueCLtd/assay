...
- [X] **T22** [P] `src/components/legends.ts`: legend entries for replay mode ("record, not present") and the tooltip depth/remainder affordance.
...
- [X] **T25** Verify: `npm run typecheck` clean, `npm test` green (395+ existing + new suites); no oracle/verdict/coverage-row change; components stay pure (SPEC-14/DEC-33); the heartbeat replays and a narrative runs end-to-end from a cold start (SC-001…004). Run the project `verify` skill on the scrubber + tooltip.

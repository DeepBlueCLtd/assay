...
**Status**: **Built (2026-07-17).** US1 (decision-history scrubber — `src/app/history.ts`, `AppState.reconstructAt`, the canonical heartbeat seed, the observer scrubber) and US2 (the five narratives re-expressed as `(seq, tab, note)` scrub paths, retiring the bespoke beat/action mechanism, the SPEC-17 presenter contract preserved verbatim) landed over research note `15-replay.md` and DEC-37/38/39. **US3 rescoped and built (2026-07-17, PR #60)** — issue #24's full dependency-graph view (DEC-47) shipped ahead of this slice, so US3 *integrates* with it (reuses its traversal, hands its counted remainders off to it) rather than pre-figuring it. US3 lives in `traceView.ts` + `components/recursiveTrace.ts`; its breadth-cap refinement was ratified 2026-07-18 as ASSAY-DEC-50 (batch 9). No schema/seam change; read-only projection over the existing substrate (DEC-10).

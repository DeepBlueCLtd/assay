...
**Status**: Built — all five moves shipped: US1 `src/components/verdictLegend.ts` (+ `src/verdictMap.ts`), US2 `src/preview.ts` (`AppState.fork` consequence preview, over research note `docs/research/14-legibility.md`), US3 `s3Cards.ts` "puts at risk" line, US4 `src/components/challengePanel.ts`, US5 `src/roleMenus.ts` + `src/components/roleMenu.ts`; wired into the live SPA and a gallery moment; 523 tests, typecheck clean. Register candidates concept §6.30/§6.31 ratified 2026-07-18 as ASSAY-DEC-48/49 (batch 9); SPEC-25 may now be cited as authority for the consequence-preview interaction class and the per-role action menus.
